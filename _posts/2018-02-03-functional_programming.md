...
什么是函数式编程？根据接触的深浅不同，每个人心里都有各自的理解，有的人认为链式调用就是函数式编程，有的认为闭包或者柯里化是。所以到底是什么呢？下面我就来谈一谈自己的浅薄理解。

## Trending
![trending](assets/img/trending.jpg)

发现2016年后主流语言Java和C以及C++有一个明显的下降趋势，Go是当年的年度语言，Swift，Kotlin，Clojure等具有函数式能力的语言有大幅的增长。那时候特别明显印象就是去参加各种技术讲座，几乎都充斥着函数式主题的内容。
